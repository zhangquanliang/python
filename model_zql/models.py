...
from sqlalchemy import Column, Integer, String, DECIMAL, DateTime, Sequence
...
# 初始化创建表
def create_table():
    from model_zql.model_manager import engine
...
class ModelProcessor:
    ...
    def from_dict(cls, **dict_):
        ...
        for k in list(dict_):
            ...
            if isinstance(fields[k], DECIMAL):
                if isinstance(dict_[k], str):
                    dict_[k] = float(dict_[k].replace(',', ''))
        return cls(**dict_)
...
class LoginAcc(BaseModel, ModelProcessor):
    ...
    id = Column(Integer, Sequence('seq_login_info'), primary_key=True)  # 主键 Sequence索引
...
class AccBaseInfo(BaseModel, ModelProcessor):
    ...
    id = Column(Integer, Sequence('seq_acc_baseinfo'), primary_key=True)  # id主键
    bank_type = Column(String(255), nullable=False)  # 银行类型
...
class AccBalance(BaseModel, ModelProcessor):
    ...
    id = Column(Integer, Sequence('seq_acc_balance'), primary_key=True)
    bank_type = Column(String(255), nullable=False)  # 银行类型
    acc_number = Column(String(255), nullable=True)  # 账号
    acc_name = Column(String(255), nullable=True)  # 账号名称
    acc_type = Column(String(255), nullable=True)  # 账号类型
    ...
    created_at = Column(DateTime, nullable=True, default=datetime.datetime.now())  # 创建时间
    updated_at = Column(DateTime, nullable=True)  # 更新时间
    deleted_at = Column(DateTime, nullable=True)  # 删除时间
...
class AccTraded(BaseModel, ModelProcessor):
    ...
    id = Column(Integer, Sequence('seq_acc_traded_list'), primary_key=True)  # 主键
