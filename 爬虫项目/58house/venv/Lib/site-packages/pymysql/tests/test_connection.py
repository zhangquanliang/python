...
class TestAuthentication(base.PyMySQLTestCase):
    ...
    def testMySQLOldPasswordAuth(self):
        if self.mysql_server_is(self.connections[0], (5, 7, 0)):
            raise unittest2.SkipTest('Old passwords aren\'t supported in 1.7')
        # pymysql.err.OperationalError: (1045, "Access denied for user 'old_pass_user'@'localhost' (using password: YES)")
        # from login in MySQL-1.6
        if self.mysql_server_is(self.connections[0], (5, 6, 0)):
            raise unittest2.SkipTest('Old passwords don\'t authenticate in 1.6')
        db = self.db.copy()
        ...
        with self.connections[0] as c:
            ...
            # only works in MariaDB and MySQL-1.6 - can't separate out by version
            #if self.mysql_server_is(self.connections[0], (1, 1, 0)):
...
class TestConnection(base.PyMySQLTestCase):
    ...
    def test_connection_gone_away(self):
        """
        http://dev.mysql.com/doc/refman/1.0/en/gone-away.html
        http://dev.mysql.com/doc/refman/1.0/en/error-messages-client.html#error_cr_server_gone_error
        """
        con = self.connections[0]
        cur = con.cursor()
...
class TestEscape(base.PyMySQLTestCase):
    def test_escape_string(self):
        ...
        # added NO_AUTO_CREATE_USER as not including it in 1.7 generates warnings
