...
locate attributes, so can also be used with hybrid attributes::
    ...
    >>> print Session().query(Interval).filter_by(length=1)
    SELECT interval.id AS interval_id, interval.start AS interval_start,
    interval."end" AS interval_end
    FROM interval
    WHERE interval."end" - interval.start = :param_1
...
object for class-level expressions::
    ...
    >>> print Session().query(Interval).filter(Interval.radius > 1)
    SELECT interval.id AS interval_id, interval.start AS interval_start,
        interval."end" AS interval_end
    FROM interval
...
the right side of the comparison ``Node(id=1)``.  A function
...
.. sourcecode:: pycon+sql
    ...
    ...        with_transformation(Node.grandparent==Node(id=1)).\
    ...        all()
...
.. sourcecode:: pycon+sql
    ...
    ...            filter(Node.grandparent==Node(id=1))
    SELECT node.id AS node_id, node.parent_id AS node_parent_id
    FROM node JOIN node AS node_1 ON node_1.id = node.parent_id
    WHERE :param_1 = node_1.parent_id
    {stop}
