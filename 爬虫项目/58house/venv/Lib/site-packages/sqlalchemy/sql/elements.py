...
def between(expr, lower_bound, upper_bound, symmetric=False):
    ...
    E.g.::
        ...
        stmt = select([users_table]).where(between(users_table.c.id, 1, 7))
    ...
    SQL expressions, as in::
        ...
        stmt = select([users_table]).where(users_table.c.id.between(1, 7))
...
class ClauseElement(Visitable):
    ...
    def __and__(self, other):
        ...
        .. deprecated:: 0.9.1 - conjunctions are intended to be
        ...
        :param bindparams:
          ...
          method on the construct before returning it.  E.g.::
              ...
              stmt = text("SELECT * FROM table WHERE id=:id",
                        bindparams=[bindparam('id', value=1, type_=Integer)])
    ...
    def bindparams(self, *binds, **names_to_values):
        ...
        using simple keyword arguments::
            ...
            stmt = stmt.bindparams(name='jack',
                        timestamp=datetime.datetime(2012, 10, 8, 15, 12, 1))

        Where above, new :class:`.BindParameter` objects
        will be generated with the names ``name`` and ``timestamp``, and
        values of ``jack`` and ``datetime.datetime(2012, 10, 8, 15, 12, 1)``,
...
class False_(ColumnElement):
    ...
    def _instance(cls):
        ...
        conjunction::
            ...
            >>> print select([t.c.x]).where(or_(t.c.x > 1, true()))
            SELECT x FROM t WHERE true

            >>> print select([t.c.x]).where(and_(t.c.x > 1, false()))
            SELECT x FROM t WHERE false
...
class CollectionAggregate(UnaryExpression):
    ...
    def _create_any(cls, expr):
        ...
        or to a subquery for others (e.g. mysql).  e.g.::
            ...
            # postgresql '1 = ANY (somearray)'
            expr = 1 == any_(mytable.c.somearray)

            # mysql '1 = ANY (SELECT value FROM table)'
            expr = 1 == any_(select([table.c.value]))
    ...
    def _create_all(cls, expr):
        ...
        or to a subquery for others (e.g. mysql).  e.g.::
            ...
            # postgresql '1 = ALL (somearray)'
            expr = 1 == all_(mytable.c.somearray)

            # mysql '1 = ALL (SELECT value FROM table)'
            expr = 1 == all_(select([table.c.value]))
...
class Over(ColumnElement):
    ...
    def __init__(
        ...
        * RANGE BETWEEN 1 PRECEDING AND 10 FOLLOWING::

            func.row_number().over(order_by='x', range_=(-1, 10))
...
class WithinGroup(ColumnElement):
    ...
    def __init__(self, element, *order_by):
        ...
        ``SELECT department.id, percentile_cont(0.1)
...
class FunctionFilter(ColumnElement):
    ...
    def over(self, partition_by=None, order_by=None):
        ...
        The expression::
            ...
            func.rank().filter(MyClass.y > 1).over(order_by='x')

        is shorthand for::

            from sqlalchemy import over, funcfilter
            over(funcfilter(func.rank(), MyClass.y > 1), order_by='x')
...
class conv(_truncated_label):
    ...
    use this explicitly as follows::
        ...
        t = Table('t', m, Column('x', Integer),
                        CheckConstraint('x > 1', name=conv('ck_t_x5')))
