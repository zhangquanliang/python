...
def parse_html(url):
    ...
    soup = BeautifulSoup(detali_code, 'html.parser')
    titles = soup.find("h1").text
    contents = soup.find("div", class_="box_con").text
    f_date = soup.find("div", class_='box01').find('div').text
    f_date = f_date.split('\xa0')[0]
    sql = "insert into news(url, titles, contents, f_date) values ('%s', '%s', '%s', '%s')" % (url, titles, contents, f_date)
    db_insert(sql)
