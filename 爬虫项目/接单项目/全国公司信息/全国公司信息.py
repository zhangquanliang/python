...
def parse_html(html):
    ...
    for table_tr in table_tr_list:
        ...
        lxr = soup2.find('table', class_='nameList').get_text().strip()
        lxdh = soup2.find('table', class_='phoneList').get_text().strip()
        dz = table_td_list[-2].get_text().strip()
        dq = table_td_list[-1].get_text().strip()
        print('111', gsmc,lxr, lxdh, dz, dq)
